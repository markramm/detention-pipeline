...
def main():
    ...
    # Enrich entries with the metadata the direct-write path used to add
    # inline, so json_to_entries.py's converter produces identical output.
    for entry in entries:
        entry.setdefault("source", "USDA ERS County Typology Codes 2025")
        entry.setdefault("source_url", "https://www.ers.usda.gov/data-products/county-typology-codes/")

    # Emit JSON only. run_ingest_ci.sh pipes this through json_to_entries.py,
    # which writes stable-slug markdown to kb/budget/ with idempotent
    # content-hash skip. No more direct writes here: keeps the whole ingest
    # atomic against git rollback.
